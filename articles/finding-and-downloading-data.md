...
Most GEOquery examples assume you already have an accession. This
article covers the steps before and around that: **finding** studies
programmatically, **controlling** what
[`getGEO()`](http://seandavi.github.io/GEOquery/reference/getGEO.md)
fetches, **caching** downloads for reproducible work, and reaching
**private** records under review.
...
If you do not yet have an accession,
[`searchGEO()`](http://seandavi.github.io/GEOquery/reference/searchGEO.md)
queries the NCBI GEO DataSets database with the standard Entrez syntax
and returns one row per hit:
...
The query uses Entrez field tags — `[Title]`, `[Organism]`,
`[Description]`, `[ETYP]` (entity type, e.g. `GSE`), and many more —
combined with `AND` / `OR`. To see the fields available for searching:
...
Once a hit looks promising, hand its accession straight to
[`getGEO()`](http://seandavi.github.io/GEOquery/reference/getGEO.md):
...
To open a record in your web browser instead — handy for eyeballing a
study before committing to a download — use
...
[`getGEO()`](http://seandavi.github.io/GEOquery/reference/getGEO.md) has
...
supplementary files (RNA-seq counts, single-cell matrices, and more) are
listed and downloaded separately. Always look before you leap:
...
(`destdir = tempdir()`), which R clears at the end of the session — so
the same
[`getGEO()`](http://seandavi.github.io/GEOquery/reference/getGEO.md)
call re-downloads every session. For interactive work and reproducible
pipelines you usually want downloads to persist. Two options:
...
**Or turn on the built-in persistent cache**, backed by `BiocFileCache`.
Once enabled, repeated downloads of the same URL are served from disk
across sessions:
...
For results you intend to publish, record what you used: pin a stable
`destdir` or the cache, and capture
[`sessionInfo()`](https://rdrr.io/r/utils/sessionInfo.html) (GEOquery
and dependency versions) alongside your output. GEO records can change
over time, so a saved object plus a version record is the difference
between reproducible and “worked on my machine.”

## Private and embargoed records

Records still under peer review are not published to GEO’s public FTP
tree, so a normal
[`getGEO()`](http://seandavi.github.io/GEOquery/reference/getGEO.md)
cannot see them. If you have a **reviewer access token** (the “Reviewer
access” link on the private GSE’s page), pass it with `token`:
...
(`acc.cgi`) download path. For a GSE that means you get a `GSE` S4
object (as with `GSEMatrix = FALSE`) rather than a
...
- [Understanding GEO data
  ...
  — what SOFT vs. Series Matrix means for the object you get back.
- [RNA-seq quantifications from
  GEO](http://seandavi.github.io/GEOquery/articles/rnaseq.md) and
  [Single-cell data from
  GEO](http://seandavi.github.io/GEOquery/articles/single-cell.md) —
  dedicated paths for sequencing-era supplementary data.
