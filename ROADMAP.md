...
| **SummarizedExperiment migration** ✅ | P1 | L | #71, #168 | DONE. `getGEO(returnType=)` added with `as_SummarizedExperiment()` coercion (ADR-0002); the default was then flipped to `SummarizedExperiment` immediately (ADR-0005) rather than staged. `ExpressionSet` remains available via `returnType = "ExpressionSet"`. |
