...
# and get the column descriptions, also
...
<<>>=
data.matrix <- log2(do.call('cbind',lapply(GSMList(gse),function(x) {Table(x)$VALUE})))
data.matrix[1:5,]
@ 

And then, to make the \Rclass{exprSet} object:

<<>>=
require(Biobase)
eset2 <- new('exprSet',exprs=data.matrix)
eset2
@ 
