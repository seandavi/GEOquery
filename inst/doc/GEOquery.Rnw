...
gse2553
pData(phenoData(gse2553[[1]]))[1:5,c(1,6,8)]
@ 
...
First, make sure that using the method described above for using GSE Series Matrix files is not sufficient for the task, as it is much faster and simpler.  If it is not (i.e., other columns from each GSM are needed), then this method will be needed.  
