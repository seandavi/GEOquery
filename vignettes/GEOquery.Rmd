...
The GEO database contains literally millions of samples and tens-of-thousands of studies. There are a number of approaches for finding data in the GEO database:

* Using the [web search functionality](http://www.ncbi.nlm.nih.gov/gds/)
* Using the `r Biocpkg("GEOmetadb")` package
* Using the included `queryGEO` function

## Example `queryGEO` usage

The `queryGEO` function takes as input a string query as described on [this page](http://www.ncbi.nlm.nih.gov/geo/info/qqtutorial.html). Very advanced querying and filtering is available. The function uses the `r CRANpkg("rentrez")` package to perform the query in the 'gds' database at NCBI. The results are returned as a data.frame. Note that the resulting GEO entities may be GPL, GDS, GSE, or GSM, so further filtering of the returned data.frame may be necessary. The `accession` column can be used in a loop in calls to `getGEO` (see below).
