...
stores, *why* it is shaped the way it is, and *how* GEOquery maps each form
onto a Bioconductor object. Understanding the formats is the fastest way to
predict what `getGEO()` will return and to debug the occasional surprising
record.

## The four GEO entity types

GEO is organized around four accession types, and every GEOquery workflow
starts by recognizing which one you have:
...
A `GSE` is the unit you almost always want. The subtlety is that a single
`GSE` can contain samples from more than one platform, which is why
...
representation. It is a line-oriented text format where metadata lines begin
with `!`, `^`, or `#`, and data tables are delimited by `!..._table_begin` /
`!..._table_end` markers. SOFT contains *everything* GEO knows about an entity.

GEOquery parses SOFT into its own S4 classes — `GSE`, `GSM`, `GPL`, `GDS` —
that mirror the file structure. You reach the pieces with accessors:
...
SOFT is the right choice when you need fields that only exist in the full
record. The cost is size and speed: a large `GSE` family SOFT file can be
enormous, and parsing it is correspondingly slow.
...
Bioconductor `ExpressionSet` (or, opting in, a `SummarizedExperiment` — see
below):
...
`ExpressionSet` (from **Biobase**) is the historical container and remains the
default. `SummarizedExperiment` is the modern Bioconductor standard and the
substrate that single-cell (`SingleCellExperiment`) and spatial
(`SpatialExperiment`) classes build on. GEOquery can return either:
...
`SummarizedExperiment` — so pick one and stay consistent. The default will move
to `SummarizedExperiment` in a future release.
...
images — is attached as **supplementary files** that GEO does not parse and
whose format it does not constrain. GEOquery lists and downloads them but, by
design, does not try to interpret arbitrary content:
...
- [Downstream analysis](downstream-analysis.html) — taking a GEOquery object
  into differential expression and beyond, with pointers to the relevant
