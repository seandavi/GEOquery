...
Bioconductor `SummarizedExperiment` (or, opting back, an `ExpressionSet` — see
below):
...
`SummarizedExperiment` is the modern Bioconductor standard and the substrate
that single-cell (`SingleCellExperiment`) and spatial (`SpatialExperiment`)
classes build on; it is now the GEOquery **default**. `ExpressionSet` (from
**Biobase**) is the historical container, still available on request:
