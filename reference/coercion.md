...
the `MAList` return type is superseded by
`ExpressionSet`/`SummarizedExperiment`, which can be coerced to whatever
downstream analysis needs. Use `GDS2eSet()` instead (and convert to an
`MAList` with limma if you still need one).
