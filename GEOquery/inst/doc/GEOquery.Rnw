...
\maketitle
\begin{center}$^\ddagger$Cancer Genetics Branch\\ National Human Genome Research Institute\\ National Institutes of Health
\end{center}
...
\section{Converting to BioConductor exprSets and limma MALists}
GEO datasets are (unlike some of the other GEO entities), quite similar to the \Rpackage{limma} data structure \Rclass{MAList} and to the \Rpackage{Biobase} data structure \Rclass{exprSet}.  Therefore, there are two functions, \Rfunction{GDS2MA} and \Rfunction{GDS2eSet} that accomplish that task.  

\subsection{Converting GDS to an exprSet}
...
Now, \Robject{eset} is an \Rclass{exprSet} that contains the same information as in the GEO dataset, including the sample information, which we can see here:
...
So, \Robject{gpl} now contains the information for GPL5 from GEO.  Unlike \Rclass{exprSet}, the limma \Rclass{MAList} does store gene annotation information, so we can use our newly created \Robject{gpl} of class \Rclass{GPL} in a call to \Rfunction{GDS2MA} like so:
...
\subsection{Converting GSE to an exprSet}
Converting a \Rclass{GSE} object to an \Rclass{exprSet} object currently takes a bit of R data manipulation due to the varied data that can be stored in a \Rclass{GSE} and the underlying \Rclass{GSM} and \Rclass{GPL} objects.  However, using a simple example will hopefully be illustrative of the technique.  
...
Note that we do a ``match'' to make sure that the values and the platform information are in the same order.  Finally, to make the \Rclass{exprSet} object:

<<>>=
require(Biobase)
# go through the necessary steps to make a compliant exprSet
